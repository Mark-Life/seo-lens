...
### Generator script
- `packages/seo-rules/scripts/generate-schema.ts` — Node script, run via `pnpm --filter @workspace/seo-rules run update-schema`.
...
- CI runs `pnpm run update-schema --check` to guarantee committed file matches current source.
...
**Actions:**
1. Download `https://schema.org/version/latest/schemaorg-current-https.jsonld`.
2. Save verbatim to the vendor path above.
3. Add a short header comment to this plan's §3 noting the version/date fetched (for future updates).
**Done when:** file exists, is valid JSON, >1MB in size, committed.
...
**Done when:** `pnpm --filter @workspace/seo-rules run update-schema` regenerates `schema-vocab.ts`; file compiles; classes set contains >800 entries.
...
**Done when:** both `KNOWN_TYPES` deleted; `pnpm test` green; no references to the removed constants.
...
2. CI runs `pnpm --filter @workspace/seo-rules run update-schema -- --check` before tests.
